...
def returnSev4(xmlpath, appname, confpath):
  ...
  for sev4 in sev4flaws:
    ...
    sev4_list[pos].affects_pol_comp   = sev4.attrib['affects_policy_compliance']
    ...
    sSql="insert into flaws (issueid,cweid,pcirelated,date_first_occurrence,remediation,mitigation,severity,categoryname,module,sourcepath,sourcefile,affects_pol_comp,gp_expires,app_name,istracked) \
    VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"  %(sev4_list[pos].flawid, sev4_list[pos].cweid, sev4_list[pos].pcirelated, sev4_list[pos].datefo, sev4_list[pos].remediation, sev4_list[pos].mitigation, sev4_list[pos].severity, sev4_list[pos].catname, sev4_list[pos].module, sev4_list[pos].sourcepath, sev4_list[pos].sourcefile, sev4_list[pos].affects_pol_comp, sev4_list[pos].gp_expires, sev4_list[pos].app_name, sev4_list[pos].istracked)
    #print sSql, '\n\n'
    pos += 1
    try:
      cur.execute(sSql)
